...
label_dellay = Label(text = "Dellay")
...
label_dellay.place(relx = 0.5, rely = 0.05, anchor="center")
...
screen.bind("<Button-1>",  lambda event:screen.focus() if event.widget  ==  screen else 0)
screen.bind("<FocusOut>",  lambda _: screen.focus())
...
def AutoClicker():
    global clicking
    dellay : float = 0.0
    for entry, multiplier in [(milliseconds, 0.001), (seconds, 1), (minutes, 60), (hours, 3600)]:
        try:
